...
// Copyright (c) 2013-2015 The FuMPCoin developers
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FuMPCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FuMPcoin Signed Message:\n";
...
{
    // FuMPCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 27000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 27000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6 * 60 * 60; // 6 hours // Actually not used, due to target diff code
static const int64 nTargetSpacing = 600; // 10 * 60;  // 10 minute block time target
...
*/	
	averageCount=7;
	if(pindexLast->nHeight < 9)
	{
		averageCount = (int)(pindexLast->nHeight - 2);
		if(averageCount< 0 ) averageCount=0;
	}
	if(averageCount >= 7)
	{
		pindexFirst = pindexLast->pprev; 	// Set previous block
		for(i=0;i<(unsigned int)averageCount;i++) pindexFirst = pindexFirst->pprev; // Set 4th previous block for 8 block filtering
		nActualTimespan = pindexLast->GetBlockTime() - pindexFirst->GetBlockTime(); 	// Get last X blocks time
		nActualTimespan = nActualTimespan / (averageCount+1); 	// Calculate average for last 8 blocks
	}
	else
	{
		nActualTimespan = 600; // set to 10 min 
	}
	...
	{
		printf("Within dead zone. No change!  error: %"PRI64d"\n", error);
...
// ********************
	...
	if(pindexLast->nHeight < 10) // Only valid for release day!
	{
		printf("Release day start difficulty.");
 		bnNew.SetCompact(0x1d0fffff); 
		return bnNew.GetCompact(); 
	}	
	
	if(DiffMode == 0) // DigiShield algo 
	{ 
		printf("DigiShield Algo\n");
		return GetNextWorkRequired_DIGI(pindexLast, pblock); 
	} 
	if(DiffMode == 1) // PID Algo 
	{ 
		printf("PID Algo\n");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FuMPCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
//        hashGenesisBlock = uint256("0xec7987a2ab5225246c5cf9b8d93b4b75bcef383a4a65d5a265bc09ed54006188");
        hashGenesisBlock = uint256("0x60a944a0c0856a7621b546ca5bfb986523b3c4acbbf1317eb4278077dfb532d4");
//    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "FuMPFest LIVE on Dementia Radio 2015-06-05 through 2015-06-07.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xa2419f214aae775c4220b1708b7b957c83a6febed28e018301023018ed915721"));
        /*
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xdc }; // FuMPCoin: increase each by adding 3 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FuMPCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FuMPCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FuMPCoinMiner(CWallet *pwallet)
{
    printf("FuMPCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("FuMPCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running FuMPCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FuMPCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FuMPCoinMiner, pwallet));
}
